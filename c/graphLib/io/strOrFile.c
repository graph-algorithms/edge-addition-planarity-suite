...
int sf_getc(strOrFileP theStrOrFile)
{
    ...
    if ((theStrOrFile->ungetBuf != NULL) && (sp_GetCurrentSize(theStrOrFile->ungetBuf) > 0))
    {
        int currChar = 0;
#ifndef SPEED_MACROS
        if (sp__Pop(theStrOrFile->ungetBuf, &currChar) != OK)
        {
            sf_SetInputErrorFlag(theStrOrFile);
            return EOF;
        }
    ...
    else if (theStrOrFile->theStrBuf != NULL && sb_GetUnreadCharCount(theStrOrFile->theStrBuf) > 0)
    {
        ...
        char *readString = sb_GetReadString(theStrOrFile->theStrBuf);
        if (readString == NULL)
        {
            sf_SetInputErrorFlag(theStrOrFile);
            return EOF;
        }
        theChar = (unsigned char)readString[0];
...
int sf_ReadSkipWhitespace(strOrFileP theStrOrFile)
{
    ...
    // Reaching ordinary EOF after whitespace is not an error and there is no
    // character to push back.
    if (currChar != EOF && sf_ungetc(currChar, theStrOrFile) != currChar)
    {
        sf_SetInputErrorFlag(theStrOrFile);
        return NOTOK;
    }
...
#ifndef SPEED_MACROS
    if (sp__Push(theStrOrFile->ungetBuf, theChar) != OK)
    {
        sf_SetInputErrorFlag(theStrOrFile);
        return EOF;
    }
...
int sf_ungets(char *strToUnget, strOrFileP theStrOrFile)
{
    ...
    strLen = strlen(strToUnget);

    if (!sf_IsValidStrOrFile(theStrOrFile) ||
        theStrOrFile->containerType != INPUT_CONTAINER ||
        strLen > INT_MAX ||
        (int)strLen > (sp_GetCapacity(theStrOrFile->ungetBuf) - sp_GetCurrentSize(theStrOrFile->ungetBuf)))
    {
        sf_SetInputErrorFlag(theStrOrFile);
        return NOTOK;
    }
...
#ifndef SPEED_MACROS
        if (sp__Push(theStrOrFile->ungetBuf, (unsigned char)strToUnget[i]) != OK)
        {
            sf_SetInputErrorFlag(theStrOrFile);
            return NOTOK;
        }
...
/********************************************************************
 sf_SetInputErrorFlag()

 Marks the container as having encountered an error while reading.
 ********************************************************************/
...
/********************************************************************
 ...
 Marks the container as having encountered an error while writing.
...
/********************************************************************
 ...
 If either operation returns a nonzero error code, marks the matching input or
 output error flag and returns NOTOK; otherwise returns OK.
 ********************************************************************/
