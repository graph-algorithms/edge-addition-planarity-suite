...
// TODO LATER: REMOVE INCLUDE_K33SEARCH_EMBEDDER and MOVE EMBEDFLAGS_SEARCHWITHEMBEDDER to be with the other EMBEDFLAGS declarations
//             (used now for easy inventory total impact of including K33SEARCH_EMBEDDER)
...
// A flag that only has meaning if used with one of the EMBEDFLAGS_SEARCH* flags
// (and if the search algorithm supports creating an embedding if the search
// does not find the desired graph structure).
