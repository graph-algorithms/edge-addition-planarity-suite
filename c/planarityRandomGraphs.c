...
int  Result;
platform_time start, end;
graphP theGraph=NULL, g6ReadFromAdjList=NULL, adjListReadFromG6=NULL, adjListReadFromG6ReadFromAdjList=NULL;
int embedFlags = GetEmbedFlags('p');
	...
	{
		ErrorMessage("No output filename given.\n");
		return NOTOK;
	}

	GetNumberIfZero(&graphOrder, "Enter number of vertices:", 1, 1000000);
	if ((theGraph = MakeGraph(graphOrder, 'p')) == NULL)
		return NOTOK;
	
	if ((g6ReadFromAdjList = gp_New()) == NULL)
	{
		messageFormat = "Failed to allocate graph structure to hold graph read from AdjList.\n";
		sprintf(messageContents, messageFormat);
		ErrorMessage(messageContents);
		gp_Free(&theGraph);
		return NOTOK;
	}

	if ((adjListReadFromG6 = gp_New()) == NULL)
	{
		messageFormat = "Failed to allocate graph structure to hold graph read from .g6.\n";
		sprintf(messageContents, messageFormat);
		ErrorMessage(messageContents);
	...
	if ((adjListReadFromG6ReadFromAdjList = gp_New()) == NULL)
	{
		messageFormat = "Failed to allocate graph structure to hold graph read from adj list read from .g6 read from adj list.\n";
		sprintf(messageContents, messageFormat);
		ErrorMessage(messageContents);
	...
	Message("Creating the random maximal planar graph...\n");
	platform_GetTime(start);

	Result = gp_CreateRandomGraphEx(theGraph, 3*graphOrder-6);
	
	platform_GetTime(end);

	if (Result == OK)
	{
		sprintf(messageContents, "Created random maximal planar graph with %d edges in %.3lf seconds.\n", theGraph->M, platform_GetDuration(start,end));
		Message(messageContents);

		strBufP g6OutfileName = sb_New(strlen(outfileName));
		sb_ConcatString(g6OutfileName, outfileName);
		sb_ConcatString(g6OutfileName, ".g6");
		...
		{
			ErrorMessage("Failed to write random maximal planar graph to AdjList file.\n");
		...
		// create additional graphP that reads GOOD adjlist, writes to g6
		strBufP adjListToG6OutfileName = sb_New(strlen(outfileName));
		sb_ConcatString(adjListToG6OutfileName, outfileName);
		sb_ConcatString(adjListToG6OutfileName, ".AdjList.g6");
		char *adjListToG6OutfileNameStr = sb_TakeString(adjListToG6OutfileName);
		sb_Free(&adjListToG6OutfileName);

		if (gp_Read(g6ReadFromAdjList, adjListOutfileNameStr) != OK)
		{
		...
		if (gp_Write(g6ReadFromAdjList, adjListToG6OutfileNameStr, WRITE_G6) != OK)
		{
			ErrorMessage("Failed to write random maximal planar graph read from Adj List to .g6.\n");
		...
		// create another additional graphP that reads GOOD g6 produced by previous step, writes to adjlist; IS IT SAME AS GOOD ADJ LIST (BARRING 0 VS 1 BASED)?
		strBufP adjListToG6ToAdjListOutfileName = sb_New(strlen(outfileName));
		sb_ConcatString(adjListToG6ToAdjListOutfileName, outfileName);
		sb_ConcatString(adjListToG6ToAdjListOutfileName, ".AdjList.g6.AdjList.out.txt");
		char *adjListToG6ToAdjListOutfileNameStr = sb_TakeString(adjListToG6ToAdjListOutfileName);
		sb_Free(&adjListToG6ToAdjListOutfileName);
		...
		// create additional graphP that reads BAD .g6, writes to Adj List
		strBufP g6ToAdjListOutfileName = sb_New(strlen(outfileName));
		sb_ConcatString(g6ToAdjListOutfileName, outfileName);
		sb_ConcatString(g6ToAdjListOutfileName, ".g6.AdjList.out.txt");
		char *g6ToAdjListOutfileNameStr = sb_TakeString(g6ToAdjListOutfileName);
		sb_Free(&g6ToAdjListOutfileName);

		if (gp_Read(adjListReadFromG6, g6OutfileNameStr) != OK)
		{
			ErrorMessage("Failed to read random maximal planar graph from .g6 file.\n");
		...
		if (gp_Write(adjListReadFromG6, g6ToAdjListOutfileNameStr, WRITE_ADJLIST) != OK)
		{
			ErrorMessage("Failed to write random maximal planar graph read from .g6 to Adj List.\n");
	...
	{
		ErrorMessage("Failed to generate a maximal planar graph.\n");
		Result = NOTOK;
	}
