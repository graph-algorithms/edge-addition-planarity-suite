...
def call_planarity_testAllGraphs(
    ...
    infile_path = Path.joinpath(input_dir, f"{stem}.g6")
    ...
    if log_root_path is not None:
        logfile_path = Path.joinpath(
            log_root_path, f"{command}", f"{stem}.{command}.log"
        )
...
def _validate_and_normalize_planarity_testAllGraphs_workload_args(  # pylint: disable=too-many-branches
    ...
    input_dir: Path,
    output_dir: Path,
) -> tuple[Path, int, tuple[str, ...], Path, Path]:
    ...
    integer in closed interval [2, 12], that commands comprises valid planarity
    command strings, that input_dir is a valid Path which corresponds to a
    directory containing .g6 files, and that output_dir is a valid Path
    specifying where results from executing planarity should be output.

    Args:
        planarity_path: Path to the planarity executable
        order: Desired number of vertices
        commands: Graph algorithm commands to run
    ...
    if not commands:
        commands = PLANARITY_ALGORITHM_SPECIFIERS()

    if isinstance(commands, str):
        commands = tuple(
            command.strip() for command in re.split(";|,", commands) if command
        )

    if any(
        command not in PLANARITY_ALLOWED_COMMAND_STRINGS()
        for command in commands
    ):
        raise argparse.ArgumentTypeError("Invalid command string(s).")
    ...
    return planarity_path, order, commands, input_dir, output_dir
...
def distribute_planarity_testAllGraphs_workload(  # pylint: disable=too-many-arguments
    ...
    Args:
        ...
        makeg_g6: Bool to indicate whether or not the .g6 input files were
            generated by planarity-backup's embedded makeg
        order: Desired number of vertices
        commands: String representing a comma-separated list of of command
            strings corresponding to graph algorithm command character and
            optional modifier to apply to .g6 input slice files.
        input_dir: Directory containing all graphs of the desired order,
            with each file containing all graphs of a specific edge-count
        ...
        log_output_streams: determines whether we throw away output streams or
            write to file
    """
    planarity_path, order, commands, input_dir, output_dir = (
        _validate_and_normalize_planarity_testAllGraphs_workload_args(
            planarity_path, order, commands, input_dir, output_dir
        )
    )
    ...
    log_root_path = (
        Path.joinpath(output_dir, "logs") if log_output_streams else None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        metavar="COMMANDS_TO_RUN",
        help="Comma-separated list of graph algorithm commands to run on the "
        "slice files.\n Defaults to "
